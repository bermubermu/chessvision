class Problem < ActiveRecord::Base
  ...
  has_attached_file :picture, :styles => { :thumb=> "100x100#", :small  => "150x150>" }, :storage => :s3, :s3_credentials => "#{RAILS_ROOT}/config/s3.yml", :url  => '/pictures/:style/:basename.:extension', :path => 'pictures/:style/:basename.:extension', :bucket => 'bucketname'
